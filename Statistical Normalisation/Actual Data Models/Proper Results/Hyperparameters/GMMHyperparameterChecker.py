...
def consolidated_loss(encTarg, gmm, classifier, lambdaSwd, batchSize):
    ...
    total = lambdaSwd * lossSwd
    return total, lossSwd
...
def TTATester(encoder, classifier, der_buffer,dataLoader, ttaLoader,adaptOpt,gmm,alpha=3,beta=1):  
    encoder_state_before = copy.deepcopy(encoder.state_dict())
    classifier_state_before = copy.deepcopy(classifier.state_dict())
    
    results = TTA(encoder,classifier,der_buffer,dataLoader, ttaLoader,adaptOpt,gmm,alpha=alpha,beta=beta)
...
def TTA(encoder,classifier,der_buffer,dataLoader,ttaLoader, adaptOpt,gmm,alpha,beta,nAdaption=150):    #Need to make this not loop through the whole dataset, maybe make it take one example of each gesture type and use that?
    bestAccuracy = 0.0
    
    ttaAcc,ttaLoss,statLosses,derLosses=[],[],[],[]
    lossFn=nn.CrossEntropyLoss()
    
    # Evaluate and store the initial accuracy before any adaptation
    preAcc, preLoss = testNetwork(encoder, classifier, dataLoader, lossFn)
    bestAccuracy = preAcc
    print(f"Pre-Adaptation Accuracy: {bestAccuracy:.2f}%")
    ...
    for step in range(nAdaption):
        ...
        with autocast():
          ...
          statLoss, loss_swd = consolidated_loss(
        ...
        if (step + 1) % 15 == 0:
            ...
            if acc > bestAccuracy:
                bestAccuracy = acc
            print(f"Step {step+1}/150 | Current Acc: {acc:.2f}% (Best: {bestAccuracy:.2f}%)")
...
def loadModels(subjectID):
    ...
    PATH = f"/home/coa23nt/EMG-SNN/Statistical-Methods/Model-Generation/Subject_{subjectID}_SLSTM_TAB"
...
def create_source_gmm(encoder, trainData, device):
    ...
    gmm = GMM(n_components=8, n_features=all_latents.size(1), device=device)
...
def SubjectChecker(loss_fn,subjectID,alpha,beta,encode=0):

    matFilePaths=fileFinder(r'/home/coa23nt/EMG-SNN/Data/DB6_s%s_a'%subjectID)+fileFinder(r'/home/coa23nt/EMG-SNN/Data/DB6_s%s_b'%subjectID)
    ...
    encoder,classifier=loadModels(subjectID)
    ...
    for testList in testListDataset:
        ...
        for param in encoder.slstm1.parameters():
            param.requires_grad = False
    
        adaptOpt=torch.optim.Adam(params=list(encoder.parameters())+list(classifier.parameters()),lr=1e-4)
        bestAccuracy = TTATester(encoder, classifier, derBuffer, testLoaderIntra, TTALoader, adaptOpt,gmm,alpha=alpha,beta=beta)
    

        # This is the critical step: saving the result for later analysis
        results_dir = "tta_tuning_results"
        os.makedirs(results_dir, exist_ok=True)
        result_filename = f"{results_dir}/s{subjectID}_sess{sessions.pop(0)}_a{alpha}_b{beta}.txt"
        with open(result_filename, 'w') as f:
            f.write(f"{bestAccuracy}\n")
...
numCombinations = len(alphas) * len(betas)
subjectIdx = inp // numCombinations
paramIdx = inp % numCombinations
alphaIdx = paramIdx // len(betas)
betaIdx = paramIdx % len(betas)
        
selectedSubject = subjects[subjectIdx]
selectedAlpha = alphas[alphaIdx]
selectedBeta = betas[betaIdx]

device = torch.device("cuda") if torch.cuda.is_available() else torch.device("mps") if torch.backends.mps.is_available() else torch.device("cpu")

lossFn = nn.CrossEntropyLoss()
SubjectChecker(lossFn,selectedSubject,selectedAlpha,selectedBeta,encode=0)
